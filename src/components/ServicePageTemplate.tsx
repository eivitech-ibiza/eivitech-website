    ...
    <>
      ...
      <section className="section-tight">
        <div className="container-x grid gap-12 md:grid-cols-2">
          <div>
            ...
            <h2 className="display-md mt-3">Alcance del servicio</h2>
          ...
          <div>
            <div className="eyebrow">Cuándo solicitarlo</div>
            <h2 className="display-md mt-3">Casos típicos</h2>
            <ul className="mt-6 space-y-3 text-muted-foreground">
              {service.whenToAsk.map((it) => <li key={it} className="border-b border-border pb-3">{it}</li>)}
            </ul>
            {service.examples.length > 0 && (
              <div className="mt-8">
                <div className="eyebrow">Ejemplos de intervenciones</div>
        ...
        <section className="section-tight bg-accent/40">
          <div className="container-x">
            <div className="flex items-end justify-between gap-6 mb-10">
              <div>
                <div className="eyebrow">Proyectos relacionados</div>
                <h2 className="display-md mt-3">Casos donde aplicamos este servicio</h2>
              </div>
              <Link to="/proyectos" className="hidden md:inline-flex text-sm text-primary hover:underline">Ver todos</Link>
            </div>
      ...
      <section className="section-tight">
        <div className="container-x">
          ...
          <h2 className="display-md mt-3 mb-10">Antes de solicitar valoración</h2>
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
            {others.map((s) => (
              <Link key={s.slug} to={`/servicios/${s.slug}`} className="rounded-sm border border-border p-5 hover:border-primary transition">
                ...
                <div className="mt-1 text-xs text-muted-foreground">Ver servicio</div>
              </Link>
            ))}
          </div>
        </div>
