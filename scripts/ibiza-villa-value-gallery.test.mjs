...
const caseStudySource = readFileSync("src/components/CaseStudyTemplate.tsx", "utf8");
...
test("Ibiza Villa Value keeps Cala Carbó across project and metadata", () => {
  ...
  assert.match(project, /folderHint: "public\/media\/projects\/casa-vadella\/"/);
...
test("Ibiza Villa Value has a caption in four languages for every non-cover image", () => {
  ...
  const galleryFiles = [...project.matchAll(/"(casa-vadella-ibiza-[^"]+\.webp)"/g)]
    .map((match) => match[1])
    .filter((file) => !file.includes("mediterranean-garden-cover"));
  const translationCalls = [...projectCaptions.matchAll(
    /tr\(\s*"([^"]+)",\s*"([^"]+)",\s*"([^"]+)",\s*"([^"]+)"\s*\)/gs
  )];
  ...
  for (const call of translationCalls) {
    assert.equal(call.slice(1).every((translation) => translation.trim().length > 0), true);
  }
});

test("gallery captions feed hero, grid and lightbox alt text on desktop and mobile", () => {
  assert.match(projectPageSource, /gallery: \[project\.cover, \.\.\.project\.gallery\.filter/);
  assert.match(captionsSource, /if \(index === 0\) return undefined/);
  ...
  assert.match(caseStudySource, /<figcaption[^>]*>[\s\S]*?\{caption\}[\s\S]*?<\/figcaption>/);
});
