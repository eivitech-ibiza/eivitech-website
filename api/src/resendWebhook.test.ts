...
test("unsubscribe campaign lookup prefers broadcast, then Resend audience, then latest recipient event", () => {
  const sql = resolveUnsubscribeCampaignSql();
  assert.match(sql, /resend_broadcast_id = \$1/);
  assert.match(sql, /resend_segment_id = \$4/);
  assert.match(sql, /crm_marketing_segments/);
